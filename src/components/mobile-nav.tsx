  ...
  return (
    <div className="md:hidden">
      ...
      <button
        ...
        className="text-wood-light hover:text-amber transition-colors p-2 -mr-2"
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-cream">
          <div className="flex items-center justify-between px-6 py-5 border-b border-wood/10">
            <Link href="/" onClick={() => setOpen(false)} className="font-serif text-xl text-wood">
              Jesper Makes
            </Link>
            <button
          ...
          <nav className="px-6 py-6 space-y-1 overflow-y-auto max-h-[calc(100vh-80px)]">
            ...
            <div className="border-b border-wood/5">
              <button
                onClick={() => setToolsOpen(!toolsOpen)}
                className="flex items-center justify-between w-full py-3 text-lg text-wood hover:text-amber transition-colors"
              >
                Tools & Links
                <svg
                ...
                <div className="pb-3 pl-2 space-y-1">
                  ...
                  {toolCategories.map((cat) => (
                    <Link
                      ...
                      onClick={() => setOpen(false)}
                      className="flex items-center gap-2.5 py-2 text-sm text-wood-light hover:text-amber transition-colors"
                    >
                      <span>{cat.icon}</span>
                      {cat.title}
                    </Link>
            ...
            {userName ? (
              <Link
                href="/account"
                onClick={() => setOpen(false)}
                className="block py-3 text-lg text-amber font-medium border-b border-wood/5"
              >
                {userName}
              </Link>
            ) : (
            ...
            <div className="flex items-center gap-6 pt-6">
              ...
              <a href="https://instagram.com/jespermakes" target="_blank" rel="noopener noreferrer" className="text-wood-light hover:text-amber transition-colors" aria-label="Instagram">
              ...
              <a href="https://tiktok.com/@jespermakes" target="_blank" rel="noopener noreferrer" className="text-wood-light hover:text-amber transition-colors" aria-label="TikTok">
