"use client";

import { useState } from "react";
import Link from "next/link";

const toolCategories = [
  { title: "Festool", slug: "festool", icon: "⚡" },
  { title: "Power Tools", slug: "power-tools", icon: "🔨" },
  { title: "Hand Tools", slug: "hand-tools", icon: "✋" },
  { title: "Plywood", slug: "plywood", icon: "🪵" },
  { title: "3D Printing & Laser", slug: "3d-printing-laser", icon: "🖨️" },
  { title: "Workshop Essentials", slug: "workshop-essentials", icon: "🔧" },
  { title: "Office & YouTube Gear", slug: "office-youtube-gear", icon: "📷" },
  { title: "Gardening & Outdoors", slug: "gardening-outdoors", icon: "🌿" },
];

export function MobileNav({
  userName,
  dark = true,
}: {
  userName?: string | null;
  dark?: boolean;
}) {
  const [open, setOpen] = useState(false);
  const [toolsOpen, setToolsOpen] = useState(false);

  return (
    <div className="md:hidden">
      {/* Hamburger button: adapts to the header's color context */}
      <button
        onClick={() => setOpen(!open)}
        className={`transition-colors p-2 -mr-2 ${
          dark ? "text-cream/70 hover:text-amber" : "text-wood-light hover:text-amber-dark"
        }`}
        aria-label="Toggle menu"
      >
        {open ? (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
          </svg>
        ) : (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        )}
      </button>

      {/* Mobile menu overlay */}
      {open && (
        <div className="fixed inset-0 z-50 bg-[#20100A]">
          <div className="flex items-center justify-between px-6 py-5 border-b border-cream/10">
            <Link href="/" onClick={() => setOpen(false)} className="font-serif text-xl text-cream">
              Jesper Makes
            </Link>
            <button
              onClick={() => setOpen(false)}
              className="text-cream/70 hover:text-amber transition-colors p-2 -mr-2"
              aria-label="Close menu"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </div>

          <nav className="px-6 py-6 space-y-1 overflow-y-auto max-h-[calc(100vh-80px)]">
            <Link
              href="/work"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              Work
            </Link>

            <Link
              href="/shop"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              Plans
            </Link>

            <Link
              href="/barn"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              Barn
            </Link>

            {/* Tools with expandable categories */}
            <div className="border-b border-cream/10">
              <button
                onClick={() => setToolsOpen(!toolsOpen)}
                className="flex items-center justify-between w-full py-3 text-lg text-cream hover:text-amber transition-colors"
              >
                Gear
                <svg
                  className={`w-5 h-5 transition-transform ${toolsOpen ? "rotate-180" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  strokeWidth={2}
                  viewBox="0 0 24 24"
                >
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
              </button>

              {toolsOpen && (
                <div className="pb-3 pl-2 space-y-1">
                  <Link
                    href="/tools"
                    onClick={() => setOpen(false)}
                    className="block py-2 text-sm font-medium text-amber"
                  >
                    All Tools
                  </Link>
                  {toolCategories.map((cat) => (
                    <Link
                      key={cat.slug}
                      href={`/tools/category/${cat.slug}`}
                      onClick={() => setOpen(false)}
                      className="flex items-center gap-2.5 py-2 text-sm text-cream/70 hover:text-amber transition-colors"
                    >
                      <span>{cat.icon}</span>
                      {cat.title}
                    </Link>
                  ))}
                </div>
              )}
            </div>

            <Link
              href="/studio"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              📐 2D Design Studio
            </Link>

            <Link
              href="/marketplace"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              🏪 Marketplace
            </Link>

            <Link
              href="/title-lab"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              🧪 Title Lab
            </Link>

            <Link
              href="/storyteller"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              🎬 Storyteller Engine
            </Link>

            <Link
              href="/box-joint-jig"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              🧰 Box Joint Jig
            </Link>

            <Link
              href="/cone-lamp"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              💡 Cone Lamp
            </Link>

            <Link
              href="/rubio/guide"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              🎨 Rubio Finish Guide
            </Link>

            <Link
              href="/rubio"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              🪵 Rubio Shop
            </Link>

            {/* Lamp Designer hidden from nav until it is finished (page still reachable at /lamp-designer) */}

            <Link
              href="/blog"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              Blog
            </Link>

            <Link
              href="/newsletter"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              Newsletter
            </Link>

            <Link
              href="/about"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              About
            </Link>

            <Link
              href="/contact"
              onClick={() => setOpen(false)}
              className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
            >
              Contact
            </Link>

            {userName ? (
              <Link
                href="/account"
                onClick={() => setOpen(false)}
                className="block py-3 text-lg text-forest font-medium border-b border-cream/10"
              >
                {userName}
              </Link>
            ) : (
              <Link
                href="/login"
                onClick={() => setOpen(false)}
                className="block py-3 text-lg text-cream hover:text-amber transition-colors border-b border-cream/10"
              >
                Sign in
              </Link>
            )}

            {/* Social links */}
            <div className="flex items-center gap-6 pt-6">
              <a href="https://youtube.com/@jespermakes" target="_blank" rel="noopener noreferrer" className="text-cream/70 hover:text-amber transition-colors" aria-label="YouTube">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814ZM9.545 15.568V8.432L15.818 12l-6.273 3.568Z"/></svg>
              </a>
              <a href="https://instagram.com/jespermakes" target="_blank" rel="noopener noreferrer" className="text-cream/70 hover:text-amber transition-colors" aria-label="Instagram">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069ZM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0Zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324ZM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881Z"/></svg>
              </a>
              <a href="https://tiktok.com/@jespermakes" target="_blank" rel="noopener noreferrer" className="text-cream/70 hover:text-amber transition-colors" aria-label="TikTok">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07Z"/></svg>
              </a>
            </div>
          </nav>
        </div>
      )}
    </div>
  );
}
